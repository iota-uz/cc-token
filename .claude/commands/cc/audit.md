...
Validates all references in `.claude/` configuration files and `CLAUDE.md` files across the project: API references,
makefile commands, file paths, agent/command names, bash syntax, and enforces architectural rules (no redundant
CLAUDE.md references, single responsibility per guide, no content duplication, bash commands in CLAUDE.md only).
Delegates validation to `Explore` agent, then applies fixes and generates report.
...
1. **Agent & Command References:** Validate CLAUDE.md orchestration patterns
   - Agent names (backtick format like `go-editor`) match .claude/agents/*.md
...
|-------------------|--------------------|--------------------------------------------|
| Agent reference   | `` `go-editor` ``  | Must match `.claude/agents/go-editor.md`   |
...
| Scaled agents     | `3x go-editor`     | Valid pattern for multiple instances       |
...
| Line number claims   | "see line 145" or "at :712"                    | ⚠️ Review   | Check line content matches claimed context          |
...
- [file:line]: `old.Function()` → `new.Function()` (✅ Fixed / ⚠️ Needs Review)

### Agent & Command References

- [file:line]: Invalid agent `` `old-agent` `` → `` `new-agent` `` (✅ Fixed)
- [file:line]: Missing command `/old-cmd` (⚠️ No replacement found)
...
- [file:line]: Old anchor syntax `patterns.md#testing-standards` → `patterns.md:145:20` (✅ Fixed / ⚠️ Needs
  verification)
...
- [file:line]: Bash command in guide file (⚠️ DISCOURAGED - move to CLAUDE.md or justify)
