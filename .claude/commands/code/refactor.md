...
allowed-tools: |
  Read, Grep, Glob, Edit, AskUserQuestion,
  Bash(go vet:*), Bash(make:*), Bash(git diff:*),
  Task(subagent_type:refactoring-expert), Task(subagent_type:go-editor),
  Task(subagent_type:debugger), Task(subagent_type:db-expert),
  Task(subagent_type:ui-editor)
---
...
Based on the approved plan and selected mode, apply refactoring changes following CLAUDE.md § 1.1 orchestration rules:

**Execution Strategy:**

1. Extract the list of opportunities from the approved plan
2. Group opportunities by category and identify agent requirements
3. Route remaining opportunities to appropriate specialized agents:
    - Presentation layer (controllers, viewmodels, templates, translations) → `ui-editor`
    - Business/infrastructure layer (services, repositories, domain) → `go-editor`
    - Database (migrations, schema, queries) → `db-expert`
4. **Always launch `refactoring-expert` LAST** after all implementation changes (§ 1.1)
